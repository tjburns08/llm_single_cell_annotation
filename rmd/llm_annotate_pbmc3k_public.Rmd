---
title: "LLM based annotation of the PBMC 3k dataset"
author: Tyler J Burns, PhD
date: "2025-02-06 - 2025-02-12"
output:
  html_document:
    toc: true         
    toc_float: true     
...
\
Richard Feynmann, "What Do You Care What Other People Think?": Further Adventures of a Curious Character
...
Let's get started by uploading the PBMC 3k dataset, and running it through Seurat's guided clustering tutorial, which can be found [here](https://satijalab.org/seurat/articles/pbmc3k_tutorial.html). If you want to see how you can do the analysis without Seurat, or understand what Seurat is doing under the hood, please go to my markdown [here](https://tjburns08.github.io/scrna_seq_piepline_pbmc_3k.html).
...
What are are going to do here is related to my command line chatbot project that I wrote about here [here](https://tjburns08.github.io/command_line_writeup.html). We could in theory take this command line chatbot and use R's system() command to run it. I did indeed do this in an earlier draft of this markdown. However, a much more user-friendly way of doing it is to just have the command for calling the LLM API directly in the workflow itself. I'll show you how to do this later.
...
Below is the function that allows you to access the LLMs from OpenRouter. It takes as input the model choice and the prompt and returns the answer as output. Note that you need to sign up for an OpenRouter API key first. You can do that by going [here](https://openrouter.ai/).
...
We will leverage the fact that the clustering I do here lines up to the clustering and annotations in the guided clustering tutorial, perhaps because the PBMC 3k dataset is sufficiently simple. 

Anyway below are the annotations from the tutorial:
...
And you can see that the LLM annotations line up with the ones from the tutorial, showing that the LLM-based annotation of the PBMC 3k dataset does in fact work.

I've confirmed in a [previous markdown](https://tjburns08.github.io/annotate_pbmc_3k.html) that the clustering scheme remains sufficiently stable that the cell types in the guided clustering tutorial remain essentially the same despite not having the exact random starting point from the tutorial.
